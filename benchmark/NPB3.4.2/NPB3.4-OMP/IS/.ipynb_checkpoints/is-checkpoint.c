...
// PAPI events
#define NUM_EVENTS 4
int events[NUM_EVENTS] = {PAPI_L1_DCM, PAPI_L2_DCM,PAPI_TLB_DM,PAPI_TLB_IM};
...
/*  Accumulate the global key population */
    ...
    all_values[thread_id][2] = values[2];
    all_values[thread_id][3] = values[3];
...
//     printf("Thread %d: L1 DCM: %lld, L2 DCM: %lld\n", thread_id, all_values[thread_id][0], all_values[thread_id][1]);
    ...
    if (write_header) {
        fprintf(file, "PAPI_L1_DCM, PAPI_L2_DCM, PAPI_TLB_DM, PAPI_TLB_IM\n");
    }
    int num_threads = omp_get_max_threads();

    // Write the collected event values for all threads
    for (int i = 0; i < num_threads; i++) {
    fprintf(file, "%lld,%lld,%lld,%lld\n", all_values[i][0], all_values[i][1], all_values[i][2], all_values[i][3]);
}
...
int main( int argc, char **argv )
{
    ...
    // const char* event_names[] = {"perf::LLC-LOAD-MISSES", "perf::LLC-STORE-MISSES", "perf::LLC-PREFETCH-MISSES"};
    // for (int i = 0; i < 3; i++) {
    //     if (PAPI_event_name_to_code((char*)event_names[i], &events[i+2]) != PAPI_OK) {
    //         fprintf(stderr, "PAPI failed to convert event name %s to event code.\n", event_names[i]);
    //         exit(1);
    //     }
    // }
...
//         exit(1);
//     }
//     int native_event1, native_event2;

//     PAPI_event_name_to_code("LLC-STORE-MISSES", &native_event1);
//     PAPI_event_name_to_code("L2_PREFETCH_MISS_L3", &native_event2);

//     retval = PAPI_add_event(EventSet, native_event1);
//     if (retval != PAPI_OK) {
//         fprintf(stderr, "Error adding LLC-STORE-MISSES: %s\n", PAPI_strerror(retval));
//         exit(1);
//     }

//     retval = PAPI_add_event(EventSet, native_event2);
//     if (retval != PAPI_OK) {
//         fprintf(stderr, "Error adding L2_PREFETCH_MISS_L3: %s\n", PAPI_strerror(retval));
//         exit(1);
//     }
